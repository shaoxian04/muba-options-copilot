...
/** The fake Supabase user id `signIn` signs in as -- `wallet.ts` scopes its own storage
 * keys by this id (`setWalletMemoryScope`), so a test reading one of those keys back
 * directly needs it too, rather than assuming the old, unscoped key still applies. */
...
export async function signIn(page: Page): Promise<void> {
  await page.addInitScript(
    ({ url, token, userId }: { url: string; token: string; userId: string }) => {
    ...
    {
      url: process.env.NEXT_PUBLIC_SUPABASE_URL ?? "https://fixture.supabase.co",
      token: FAKE_ACCOUNT_TOKEN,
...
/**
 * The sealed-bid auction, faked (ADR-0017).
 *
...
export async function stubApi(page: Page, scenario: Scenario = "normal"): Promise<Traffic> {
  ...
  await page.route(`${API}/**`, async (route, request) => {
    ...
    switch (url.pathname) {
      ...
      /**
       * The RFQ money path (ADR-0017). Both doors, and all seven routes.
       *
