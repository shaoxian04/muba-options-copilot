 ...
 */
export async function realHoldings(prices: Record<string, number>): Promise<[Holding[], string | null]> {
  ...
  try {
    ...
    return [positions.filter((p) => p?.side !== "seller").map((p) => toHolding(p, prices)), address];
  } catch {
